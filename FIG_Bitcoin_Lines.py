...
btc['daystotal'] = btc.groupby('date').cumcount() + 1
...
def update(frame):
    ...
    for start, end, color in regions:
        ax.axvspan(start, end, color=color, alpha=0.05)
    ...
    ax.set_xlim(0, 1500)
    ax.set_ylim(8, 12)  # Adjust the y-axis limits as needed
    ax.set_xticks(range(0, 1501, 125))
    ax.set_yticks(np.arange(8, 12.1, 0.5))

    # Custom legend
    legend = ax.legend(title="Halving", loc='lower right', fontsize=8, title_fontsize='10')
    new_title = 'Dates:'
...
# Crear la animación
ani = FuncAnimation(fig, update, frames=btc['daystotal'].unique(), interval=0.1)
